...
require_login(); // Harus login untuk menambah buku
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($error)) {
        try {
            // Insert ke database
            $stmt = $pdo->prepare("INSERT INTO books (title, author_id, genre_id, publication_year, isbn, description, rack_id, total_copies, available_copies, cover_image, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())");
            $stmt->execute([$title, $author_id, $genre_id, $publication_year, $isbn, $description, $rack_id, $total_copies, $available_copies, $cover_image]);

            echo "<div class='alert alert-success'>Buku berhasil ditambahkan!</div>";
            echo "<a href='index.php'>Kembali ke Daftar Buku</a>";
        } catch (PDOException $e) {
            echo "<div class='alert alert-error'>Gagal menambahkan buku: " . $e->getMessage() . "</div>";
        }
    } else {
        echo "<div class='alert alert-error'>$error</div>";
    }
} else {
    echo "<h2>Tambah Buku Baru</h2>";
    ...
    echo "<input type='number' id='publication_year' name='publication_year' min='1000' max='2030' required>";
    echo "</div>";
    ...
    foreach ($racks as $rack) {
        echo "<option value='{$rack['id']}'>{$rack['name']}</option>";
    }
    echo "</select>";
    echo "</div>";
    ...
    echo "<input type='number' id='total_copies' name='total_copies' min='1' value='1' required>";
    echo "</div>";

    echo "<div class='form-group'>";
    echo "<label for='available_copies'>Jumlah Tersedia:</label>";
    echo "<input type='number' id='available_copies' name='available_copies' min='0' value='1' required>";
    echo "</div>";
    ...
    echo "<div class='form-group'>";
    echo "<input type='submit' value='Simpan Buku' class='btn btn-primary'>";
